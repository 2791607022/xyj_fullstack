...
### React State
    -概念： React State 是一个组件的显示形态, 可以由 内部状态 和 外部参数 所决定, 外部参数 指的则是 props 而 内部状态 则是 state, 同时需要注意的是只有通过 setState 或者 useState 中指定的方法修改状态才会触发 render

    - 更新机制：React 18之前，在组件生命周期或 React 事件中, setState 是异步，在 setTimeout/setInterval 或者原生 dom 事件中, setState 是同步；本质上来讲 setState 是同步的, 之所以出现异步的假象是因为要进行 状态合并 或者说是 批处理, 需要等生命周期、事件处理器执行完毕, 再批量修改状态! 
    
    - setState机制：React 的 setState 函数实现中, 会根据一个变量 isBatchingUpdates 判断是直接更新 this.state 还是放到队列中进行处理。isBatchingUpdates 默认是 false, 当 React 在执行生命周期或调用事件处理函数之前会将其设置为 true, 当执行完生命周期或者事件处理函数再改为 false 然后才会一起更新状态、更新组件, 所以整个过程看起来像异步的。在原生事件中, 由于不会调用 React 批处理机制, 所以 isBatchingUpdates 一直是 false, 所以如果调用 setState 会直接更新 this.state, 整个过程看起来就像是同步

    - 更新： React 18之后，实现自动批处理, 所以不管任何情况所有状态的修改都会进行批处理，所有状态的修改都将是 "异步" 的，原因:
        1.为了保证state 和 props 的一致性，因为 props 必然异步，只有当父组件重渲染了我们才知道 props的更新，所以将state的更新方法与props一致
...
### react 合成事件
    - React合成事件是指将原生事件合成一个React事件，之所以要封装自己的一套事件机制， 是因为原来的浏览器事件模型分为捕获、冒泡两种模式，为了实现全浏览器的一致性，抹平不同浏览器之间的差异性，React实现了一套自己的事件模式来处理这些情况，比如原生onclick事件对应React中的onClick合成事件

    - 在事件注册阶段， React会调用listenToNativeEvent把这个事件的优先级分为两类，一类是 allNativeEvents 既浏览器支持的所有交互事件，另一类是只支持在捕获阶段被触发的事件，分类后就通过 createEventListenerWrapper 这个函数将不同类的事件匹配对应的dispatchEvent方法，然后根据不同的dispatchEvent 调用不同的addEventListener 方法来给id为root的dom节点进行事件绑定，这样事件的注册流程就结束了

    - 在事件执行阶段，比如页面上有个按钮点击后，页面的值加1，因为所以事件都在root节点上绑定了一遍，根据事件委托的性质，此时会触发root上的click绑定的dispatch事件，在寻找阻塞事件实例这个方法中会通过递归方式拿到事件触发时的组件，根据组件类别不同做不同处理，但最终会进入更新阶段，React的函数 会收集对涉及到的事件并将它放入批量更新的队列中，最后批量执行事件。

    - 关于合成事件和原生事件的执行顺序：
        这个顺序由原生事件的触发时机是冒泡阶段还是捕获阶段所决定，如果这个原生事件A的默认触发时机是冒泡阶段，那么当一个组件既绑定了合成事件，又绑定了原生事件，此时原生事件的执行时机先于合成事件；原生事件的默认触发时机是捕获阶段，那么合成事件的执行时机先于原生事件

### render props
    - 简述：render props 是指在 React 组件中使用一个值为函数的属性(props)来渲染代码块的技术,组件允许通过属性传入一个函数, 该函数返回一个 React 元素,通过调用该函数, 来渲染部分内容;在 调用函数时允许为函数传递任意参数, 可以是组件内部状态、方法、或其他任意数据。

    - 优点：render 函数可以通过参数 拿到和调用组件内部状态、方法、任意数据；方便进行组件的复用, 把组件无关的视图渲染逻辑抽象出来, 交给用户自己定义

    - 缺点：如果在 render 方法里直接创建函数, render prop 会使得 PureComponent 或 shouldComponentUpdate 无效, 因为每次 render 总会重新创建函数, 导致浅比较总是返回 false
...
#### 2.常用Hooks 介绍
        - 使用原则：
            只在最顶层使用 Hook，不要在循环，条件或嵌套函数中调用 Hook， 确保总是在你的 React 函数的最顶层调用他们；只在 React 函数中调用 HooKs
...
### Redux
#### 介绍：
    redux本质上是一种数据管理的设计思想，它实现了Flux 模式，是应用程序的可预测状态管理模式。通过引入单向数据流和应用程序状态的集中存储来帮助管理应用程序的状态它的出现就是为了解决react组件的状态管理。redux内部管理了一个状态树（state），根据开发者提供的reducer来“派发”一个“动作”以更新state，这样数据管理全部交由redux来处理而不在由react组件去操心。它有几个核心概念：
...
### 高阶组件
    本质: 本质上就是一个函数, 是一个参数为组件, 返回值为新组件的函数
    高阶组件内部实现方式:
...
### react 优化实践
    ...
    4.将页面进行更小的颗粒化，如果一个过大，当状态发生修改的时候，就会导致整个大组件的渲染，而对组件进行拆分后，粒度变小了，也能够减少子组件不必要的渲染

### 错误边界
    - 概念：错误边界是一种 React 组件, 这种组件可以捕获发生在其子组件树任何位置的异常, 我们可以针对这些异常进行打印、上报等处理, 同时渲染出一个降级(备用) UI, 而并不会渲染那些发生崩溃的子组件树。错误边界目前只在 Class Component 中实现了，没有在 hooks 中实现
...
### Fiber架构
    ...
    它把 render/update 分片, 拆解成多个小任务来执行, 每次只检查树上部分节点, 做完此部分后, 若当前一帧 (16ms) 内还有足够的时间就继续做下一个小任务, 时间不够就停止操作, 等主线程空闲时再恢复

    它根据一个 fiber 节点 (VDOM 节点) 进行来拆分, 以 fiber node 为一个任务单元, 一个组件实例都是一个任务单元, 任务循环中, 每处理完一个 fiber node, 可以中断/挂起/恢复

    不同的任务分配不同的优先级, Fiber 根据任务的优先级来动态调整任务调度, 先做高优先级的任务


### JSX
    -概念：JSX是react的语法糖，它允许在html中写JS，它不能被浏览器直接识别，需要通过webpack、babel之类的编译工具转换为JS执行
    ...
    - React自定义组件首字母要大写的原因
        jsx到真实DOM需要经历jsx->虚拟DOM->真实DOM。如果组件首字母为小写，它会被当成字符串进行传递，在创建虚拟DOM的时候，就会把它当成一个html标签，而html没有app这个标签，就会报错。组件首字母为大写，它会当成一个变量进行传递，React知道它是个自定义组件就不会报错了
