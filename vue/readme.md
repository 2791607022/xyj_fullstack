...
#### 单组件生命周期
    ...
    - 渲染阶段：
        1.beforeMount，此时内存中已经编译好了模板，但组件实例还没有挂载到DOM 上，相关的 render 函数首次被调用
...
#### Vue3 生命周期变化
    ...
    - 渲染阶段：
        onBeforeMount取代 beforeMount;onMounted取代mounted;
    – 更新：
        onBeforeUpdate 取代beforeUpdate;onUpdated取代updated;
    – 卸载阶段:
        onBeforeUnmount 取代 beforeDestroy;onUnmounted取代destroyed;
    - 其他：
...
### Vue组件通信
#### 父子组件通信：
        1.props / $emit方式： 父组件 data 里定义一个属性 a, 这个属性 a 可以作为 props 传递给子组件，子组件中的props属性中只需同一个同名的变量接受，即可访问父组件中的这个值，子组件可以听过emit 的方式将 自己的值传给父组件，子组件中可以设置一个函数b,在b中使用vue的$emit 方式将 自己的事件和数据传递给父组件，父组件可以在子组件上绑定一个函数 监听emit中设置事件，当子组件事件执行时，父组件就可以在函数中获取到传递过来的值。
        ...
        2. $parent和$children 方式：可以通过vue 上的$parent和$children 来访问组件的实例，拿到实例就可以访问此组件的所有方法和data。但这种方式可以会无意中改变实例上的东西，非常不安全，不建议使用
          ...
          如果一个组件是个子组件，它可以通过 this.$parent 访问到父实例，拿到父组件的数据和方法，组件没有父组件，他的$parent为undefined，如果该组件有多个父亲，$parent只能找到上一个。
        ...
        4.ref/refs
            和 parent和children 的原理类似，都是通过访问组件实例来获取数据，当ref作用于组件时，它就可以将组件的引用信息注册到其父组件的refs对象上，父组件通过this.$refs加上 子组件上设置ref名称就可以访问到它。这种方式也只能单向传递，而且不建议使用。当EventBus注册在全局上时，需要在使用$on的组件中在beforeDestroy钩子函数中用$off销毁事件监听，避免重复触发事件。
...
### MVVM
    - 说明：MVVM 作为一种设计模式是Model-View-ViewModel缩写，也就是把MVC中的Controller演变成ViewModel。model中定义数据修改和操作的业务逻辑;view表示ui组件，负责将数据模型转换为ui展现出来，它做的是数据绑定的声明、 指令的声明和事件绑定的声明;而viewModel是一个同步view和model的对象。在mvvm框架中，view和model之间没有直接的关系，它们是通过viewModel来进行交互的。mvvm不需要手动操作dom，只需要关注业务逻辑就可以了。
    ...
    - 实现原理：MVVM实现的核心原理是数据劫持和发布订阅模式
        1.首先 Vue在Observer类中来管理响应式化的过程，在其中会遍历data选项中所有的属性，并传入defineReactive 函数中，在该函数中使用Object.defineProperty劫持这些属性并设置属性的getter和setter函数。在获取属性时会执行getter函数，其中会把调用此属性的依赖收集到一个集合中；而在给属性赋值时，会触发setter函数，其中会去通知集合中的依赖更新，做到数据变更驱动视图变更，完成数据的响应式绑定

        2.当完成响应式后，Vue需要知道是哪些相关的视图应该更新，作为依赖管理器的Dep 函数就是用于解决这个问题。Dep 接收静态属性 target，它是订阅者依赖的数据，其中会有两个方法，addDep 方法会将传入的对象添加到Dep里的依赖数组中，update 方法是数据改变时触发更新的回调函数，同时 Dep 使用addSub 与 removeSub 用来向依赖数组中添加或删除依赖，使用depend 方法保证当前的Dep.target 不为空，并将非空的依赖传入addDep方法；使用 notify方法触发所有依赖订阅者的更新操作，通过这些方法完成依赖管理和触发依赖的更新操作。

        3.最后进行的是依赖者Watcher 的操作，组件在挂载过程中都会new一个Watcher实例。这个实例就是依赖（订阅者）。Watcher 中首先会将 Dep.target 指向当前的 watcher 实例，保证收集到的依赖与watcher 实例对应，然后会触发属性的getter方法，使用depend() 收集依赖。在数据更新的时候，会通过 setter 方法来处理，触发dep.notify()来遍历依赖数组并执行每个依赖的更新方法，最后在 run 函数中将新值替换旧值，将新值传入渲染处理，完成了响应式过程。

    - vue2 的数组的响应式处理：
        通过dependArray 遍历每个数组元素，递归处理数组项为对象的情况，为其使用dep.depend方法添加依赖，然后拦截对那些会改变原数组的方法，在添加，删除元素或排序，反时数组进行响应式处理，使用 dep.notify 通知 对应的watcher 进行数据更新。

### SPA单页面
    - 介绍：SPA是一种只需要将单个页面加载到浏览器器之中的web应用程序。当浏览器向服务器发出第一个请求时，服务器会返回一个index.html文件，它所需的js，css等会在显示时统一加载，部分视图按需加载。url地址变化时不会向服务器再请求新的页面，而是通过路由切换触发Js函数来刷新Dom，实现视图的切换，营造出一种传统多页面应用的感觉。
    - 优点：
    ...
    - SPA首屏加载优化：
        1.使用CDN资源：将需要使用的npm库，js 文件，ui框架等从代码中分离出来，使用CDN资源引入，在 index.html 用 script 标签引入对应的资源地址

        2.对代码资源进行Gzip 压缩，代码中引入压缩插件并且在代理中进行 Gzip配置。

        3.进行图片资源的压缩。进行静态资源本地缓存，减少服务器压力

        4. 进行组件和UI框架按需加载。

        5. 服务端渲染：组件或页面通过服务器生成html字符串，再发送到浏览器，浏览器可以快速直接渲染。vue应用可以使用Nuxt.js（那可扯），react 可以使用 Next.js。
### Vuex
    - 五个核心概念:state、getters、mutations、actions、modules。
        ...
        - Module 用于store模块化，Vuex 允许我们将 store 分割成模块。每个模块拥有自己的 state、mutation 等，最后将多个状态树再组合成完整的状态树

    - 相关辅助函数：
        - mapState 支持在组件中批量使用Vuex的state状态
...
### watch和computed的异同
    ...
    - watch 可以对data里的数据监听，当依赖的data的数据变化时, 会执行回调设置的回调函数，回调函数中有newVal和oldVal两个参数可以获取到新旧两个状态，在回调函数中可以执行自定义的操作，watch 不会在页面第一次加载时执行，在回调外部需要设置属性 immediate 为 true, 它才会立即执行；另外还有一个属性为 deep,当设置为 true 时，watch能对监听的数据进行深度监听，适合引用类型的数据；watch 适合某个数据发生变化时, 我们需要做一些操作, 比如需要在数据变化时执行异步或开销较大的操作时
...
### css scoped
    ...
    3.问题：如果组件内部还有组件，那只会给最外层组件加唯一属性字段，不影响组件内部子组件的样式，会导致父组件无法直接修改子组件的样式，这时我们可以进行进行样式穿透，在 sass和less中可以使用 /deep/ 处理，在  stylus中可以使用 >>> 来进行。
...
### vue2中数组更新问题
    - 数组项改变页面没更新的原因：Object.defineProperty 本身是可以循环数组并劫持数组项并监控到数组下标的变化的，只是在性能/体验的性价比的考虑下，vue 放弃了这个特性。
    - 解决方法：
        1.重新给整个数组赋值，改变数组引用地址，this.a = [...a] 或object.assign();
        2.使用vue.$set() 方法，它可以设置对象或数组的值，通过key或数组索引，可以触发视图更新。set 原理:先判断参数是数组还是对象，是对象就使用object.defineproperty()手动的去劫持实现响应式，是数组就传入的下标是否合法，然后用重写的splice改变原数组并触发响应式。
        3.使用数组中能改变原数组的方法去操作数组项变化，vue2 对这些方法进行了拦截并实现了响应式。
### vue-router
